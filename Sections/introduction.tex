...
\begin{frame}{Definion of Polyolefins (PO)}
...
\begin{frame}{Why poliolefin are so popular ?}
    ...
    \begin{figure}
        ...
        \end{figure}
    Naphta $\rightarrow$ Ethylene, propylene \textbf{or} gasoline (oct. 95 or+) depending on the \textbf{Market}. 
...
\begin{frame}{because the raw materials are cheap}
    ...
    \end{block}
     \begin{figure}
        ...
        \end{figure}
    \begin{alertblock}{natural gas (95\% $CH4$ + 3\% $C_2h_6$)}
        can also be converted into Ethylen with higher yield (shale gas of US Golf cost). \cite{noauthor_composition_nodate}
...
%
\begin{frame}{Because PO are versatiles}
    ...
    \begin{block}{for thermoplastics}
        $M_w + \text{ DPI } + \text{ branching } + \text{ Tacticity } \= \xi_c$ \& properties ($E$,$\rho$ $\sigma$  and $T_g$) 
    \end{block}
...
\begin{frame}{PO exhibit excellent chemical resistance}
    \begin{figure}
        ...
        \caption{Functionnal group reactivity (adapted from LMAPR1230).}
    \end{figure}
\end{frame}
...
\begin{frame}{PO is mainly use for short term usage}
    \begin{figure}
            \centering
